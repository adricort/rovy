...
Desarrollado por: Ricárdez Ortigosa Adrián y Ayala Almeda Alejandro
...
int PWM_Izq= 191, PWM_Der= 191;       // Variables para los motores de Rovy, empiezan en "PARO"
const int Mot_Izq = 9, Mot_Der = 10;  // Pines digitales del Arduino para los PWM de los Motores
const int pulso_error_rel = 8;        // Pin que recibirá el pulso del otro Arduino si ya llegó al 80% de error relativo
const int LED_calibrado = 12;         // LED que indicará si ya se calibró el MPU6050
const int pulso_camara_check = 11;    // Pin que enviará el pulso de terminación de la cámara
boolean IMU_calibrada = false;        // Variable booleana que indica que ya se calibró el MPU6050
boolean calculo_offset = false;       // Variable booleana que indica si ya se hizo el cálculo del offset del ángulo 
// ============== VARIABLES CÁMARA =================
int PWM_cte = 224;
int Cte_4 = 10;
int Decremento_1;
...
void setup(){
 ...
 pinMode(pulso_error_rel,INPUT);
 pinMode(LED_calibrado,OUTPUT);
 pinMode(Mot_Izq, OUTPUT);
 pinMode(Mot_Der, OUTPUT);
 pinMode(pulso_camara_check,OUTPUT);
 digitalWrite(pulso_camara_check,LOW);
}
// ===========================================
// ============== VOID LOOP ==================
// ===========================================
void loop() {
  ...
  thetha_grados= 90;//Arduinos.parseFloat(); // Lee únicamente los números flotantes que recibe del Primer Arduino del Arduinos.print(theta_grados)
  // Conversión de ángulos de "0° a 360°" a "0 a 180° y -180° a 0°"
  if( thetha_grados > 180 ){
  thetha_grados= thetha_grados-360;}
  /*  COMENTO ESTO PORQUE SI NO SE PONE LOCO EL ARDUINO XD
  ...
  while(true){
    MPU_6050();
    angulo_z= angulo_z + offset_Norte;  // El offset generalmente es como 46°
    // Conversión de angulos del gyro de "0 a 360°" a "0 a 180° y -180° a 0°"
    if( angulo_z >= 0){
      alfa_grados= angulo_z*(-1)+180;}
    else{
    ...
    //Serial.print(",");
    Serial.println(PWM_Izq);//Serial.print(",");
    //Serial.println(PWM_Der);

    // ============== CÁMARA ================
    if( digitalRead(pulso_error_rel) == HIGH ){   // SI YA LLEGÓ AL 80% DE ERROR RELATIVO DEL PRIMER ARDUINO
      ...
      while(true){
        if(Serial.available()){           // Nos dice si hay datos dentro del buffer
          ...
          if((valor > 100) && (valor <= 300)){    // VIO EL CONO HACIA LA IZQUIERDA
            ...
            Serial.println(valor);
            delay(1500);    // Se espera segundo y medio para sacar el cono de su vista
            digitalWrite(pulso_camara_check,HIGH);
            analogWrite(Mot_Der, 191);
            analogWrite(Mot_Izq, 191);
            break;  // Break que rompe while true de la cámara
          }
      ...
      }// END WHILE TRUE de la Cámara
    }// END IF (PULSO DE ERROR RELATIVO)
    //break;    // Break que rompe el while true de la POTENCIA
