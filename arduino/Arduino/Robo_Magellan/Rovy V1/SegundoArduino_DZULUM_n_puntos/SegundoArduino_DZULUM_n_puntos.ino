...
void loop() {
  ...
  while(true){
    ...
    PWM_Izq= 220+(alfa_grados-thetha_grados)*0.717;    // 0.717 fue el factor más grande para que avance 253 PWM a 60° de diferencia entre ángulos
    PWM_Der= 220+(thetha_grados-alfa_grados)*0.717;    // 0.717 fue el factor más grande para que avance 253 PWM a 60° de diferencia entre ángulos
    ...
    //Serial.println(alfa_grados);//Serial.print(",");
    ...
    if( digitalRead(interruptor) == HIGH ){
      ...
      while(true){
        if(Serial.available()){ //Nos dice si hay datos dentro del buffer
          ...
          if((valor > 100) && (valor <= 300)){      // Busca el cono hacia la izquierda si lo está detectando
          ...
          if((valor >= 0) && (valor <= 30)) {    // Busca el cono hacia la derecha si lo está detectando
          ...
          if(valor == 1000){       // Para el robot una vez que se saturó la imagen del color buscado, se espera un segundo y gira a la derecha
        ...
        }   //  END if(Serial.available())   (Donde empieza la cámara)
      }   //  END WHILE(TRUE){}
      break;
    }   // END if( digitalRead(interruptor) == HIGH )
  }   //  END WHILE(TRUE){}
} // END LOOP
