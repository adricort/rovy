...
const float k_recorrido = 1.15;   // valor en estacionamiento 1.2//valor en casa 1.23        // Constante para el cálculo de la distancia recorrida
...
unsigned long origen_lat = 3741100;//inicio livermore//1918186;//1918196; inicio casa//1918186;  // coordenada estadio
unsigned long origen_long = 12147007;//inicio livermore//9909136;//9909170; inicio casa//9909136;  // coordenada estadio
...
INICIO:
  {
    ...
    for (int i = 1; i <= n; i++) { // Ingreso de datos de orden n
      ...
      latitudes[1] = 3741103;//1918202; coladera casa//1918171; roca estadio
      latitudes[2] = 3741105;//1918212; esquina casa//1918166; cono estadio
      latitudes[3] = 3741109;//1918202; coldera casa//1918169; punto poste estadio
      latitudes[4] = 3741113;//1918202; coldera casa//1918169; punto poste estadio
      latitudes[5] = 3741123;//1918202; coldera casa//1918169; punto poste estadio
      latitudes[6] = 3741113;//1918202; coldera casa//1918169; punto poste estadio
      latitudes[7] = 3741117;
      ...
      longitudes[1] = 12147016;//9909167; coladera casa//9909139; roca estadio
      longitudes[2] = 12147033;//9909164; esquina casa//9909125; cono estadio
      longitudes[3] = 12147033;//9909167; coladera casa//9909118;  punto poste estadio
      longitudes[4] = 12147046;//9909167; coladera casa//9909118;  punto poste estadio
      longitudes[5] = 12147038;//9909167; coladera casa//9909118;  punto poste estadio
      longitudes[6] = 12147049;//9909167; coladera casa//9909118;  punto poste estadio
      longitudes[7] = 12147069;
    ...
    /*if( GPS_calibrado == false ){
      for(int j=0;j<8;j++)
      {
        leerGPS();
        if(bandera_pais == false)
        {
          latitudes[0] = lat;       // La coordenada inicial del robot tomará el valor de la lectura del GPS
          longitudes[0] = lon;       // La coordenada inicial del robot tomará el valor de la lectura del GPS
        }
        else if(bandera_pais == true)
        {
          latitudes[0] = lat;       // La coordenada inicial del robot tomará el valor de la lectura del GPS
          longitudes[0] = lon;       // La coordenada inicial del robot tomará el valor de la lectura del GPS
        }
      ...
      if(fix == 1)  //DESCOMENTAR*
      {
      Serial.println("Obtención de posición inicial exitosa:"); //DESCOMENTAR*
      Serial.print("19 ");Serial.print((latitudes[0]-lat_cte_MEX)/1000,3);Serial.print("N,");
      Serial.print("99 ");Serial.print((longitudes[0]-long_cte_MEX)/1000,3);Serial.println("W");
      }
      else //DESCOMENTAR
      { //DESCOMENTAR*/
...
CALCULO:
  {
    ...
    for (int i = 1; i <= n; i++)
    {
      L_GPS[i - 1] = sqrt(pow(latitudes[i] - latitudes[i - 1], 2) + pow(longitudes[i] - longitudes[i - 1], 2)) * k_L_GPS-1.5; // ejemplo: pow(2,4) = 2^4
      
      // Agregarle metros para que mientras más grande sea, recorra más distancia
      
      /*if(L_GPS[i - 1] > 0 && L_GPS[i - 1] < 15)
        L_GPS[i - 1] -= 1.5;
      else if(L_GPS[i - 1] >= 15 && L_GPS[i - 1] < 30)
        L_GPS[i - 1] -= 1;
      else if(L_GPS[i - 1] >= 30 && L_GPS[i - 1] < 45)
        L_GPS[i - 1] -= 0.5;
      else if(L_GPS[i - 1] >= 45 && L_GPS[i - 1] < 60)
        L_GPS[i - 1] -= 0;
      else if(L_GPS[i - 1] >= 60 && L_GPS[i - 1] < 75)
        L_GPS[i - 1] += 1;
      else if(L_GPS[i - 1] >= 75 && L_GPS[i - 1] < 90)
        L_GPS[i - 1] += 2;
      else if(L_GPS[i - 1] >= 90 && L_GPS[i - 1] < 105)
        L_GPS[i - 1] += 3;
      else if(L_GPS[i - 1] >= 105)
        L_GPS[i - 1] += 4;*/
...
ALINEACION:
  {
    ...
    } while ( abs(error_angulo) > 15);      // Lo hará mientras el abs del error sea mayor a 5 (hasta que esté semi-alineado)
...
NAVEGACION:
  {
    ...
    if ((currentMillis - previousMillis) >= interval)
    {
      ...
      if (L_enc >= L_GPS[p] || abs(L_enc - L_GPS[p]) <= 1.5 )  // Si ya llegó su destino o está por llegar
      {
      ...
      else        // Si no ha llegado a su destino
      {
        ...
        if ( (L_GPS[p] - L_enc) < 7 )  // Cuando le falten 7 metros para llegar
        {
          Serial.println("YA CASI LLEGO A MI DESTINO");
          interval += 900;
        }
        else
        {
  ...
  BUSCAR_CONO:
  {
    ...
    do
    {
      ...
      leerGPS();
      leerGPS();
    } while (fix != 1 || satel < 5 || lat == 0 || lon == 0 || lat < 3700000 || lon < 12100000 || lat > 3799999 || lon > 12199999 );  // leer el GPS mientras no se tenga una lectura confiable
...
CALCULO_AUX:
  {
    ...
    L_GPS[p] = sqrt(pow(latitudes_totales[p], 2) + pow(longitudes_totales[p], 2)) * k_L_GPS -1.5; // ejemplo: pow(2,4) = 2^4

      // Agregarle metros para que mientras más grande sea, recorra más distancia
      
      /*if(L_GPS[p] > 0 && L_GPS[p] < 15)
        L_GPS[p] -= 1.5;
      else if(L_GPS[p] >= 15 && L_GPS[p] < 30)
        L_GPS[p] -= 1;
      else if(L_GPS[p] >= 30 && L_GPS[p] < 45)
        L_GPS[p] -= 0.5;
      else if(L_GPS[p] >= 45 && L_GPS[p] < 60)
        L_GPS[p] -= 0;
      else if(L_GPS[p] >= 60 && L_GPS[p] < 75)
        L_GPS[p] += 1;
      else if(L_GPS[p] >= 75 && L_GPS[p] < 90)
        L_GPS[p] += 2;
      else if(L_GPS[p] >= 90 && L_GPS[p] < 105)
        L_GPS[p] += 3;
      else if(L_GPS[p] >= 105)
        L_GPS[p] += 4;*/
