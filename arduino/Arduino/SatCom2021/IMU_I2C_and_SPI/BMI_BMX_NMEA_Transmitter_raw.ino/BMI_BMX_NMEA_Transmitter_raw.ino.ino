...
byte seconds_rtc = 0;
const byte minutes_rtc = 4;
const byte hours_rtc = 19;      // UTC is = ECT - 2

int i = 0;

// Change these values to set the current initial date 
const byte weekDay_rtc = 3;
const byte day_rtc = 30;
const byte month_rtc = 6;
const byte year_rtc = 21;
...
void setup() {
  ...
  //rtc.setDate(weekDay_rtc, day_rtc, month_rtc, year_rtc);
  ...
  if (!rtc.isTimeSet()) {
    ...
    te.Second = seconds_rtc;
    te.Minute = minutes_rtc;
    te.Hour = hours_rtc;
    te.Day = day_rtc;
    te.Month = month_rtc;
    te.Year = (2000+year_rtc) - 1970;
  }
  else
  {
    // convert a date and time into unix time, offset 1970
...
void loop() {
  ...
  frame.concat(long(unixTime)); frame.concat(",");
