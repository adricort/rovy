...
const int u = 9;                                    // Número de sensores
const int TriggerPin[u] = {2,5,7,9,11,A4,13,A3,A1};    // Pin de emergencia: sustituir alguno con A4
const int EchoPin[u] = {3,4,6,8,10,A5,12,A2,A2};       // Pin de emergencia: sustituir alguno con A5
unsigned long Duration[u] = {0,0,0,0,0,0,0,0,0};      // La duración del Echo por el Trigger
long Distancia_mm[u] = {0,0,0,0,0,0,0,0,0};           // La distancia devuelta de la función fDistancia()
...
float Suma[u] = {0,0,0,0,0,0,0,0,0};        // Sumas del primer y segundo filtro
float Promedio[u] = {0,0,0,0,0,0,0,0,0};    // Promedios del primer y segundo filtro
...
void loop() {  
  ...
  if( Primera_Fase == false ){
    ...
    if(Promedio[0] >= distancia_min && Promedio[0] <= distancia_max)
      Promedio[0] = -1;
    else
      Promedio[0] = 0;
    if(Promedio[1] >= distancia_min && Promedio[1] <= distancia_max)
      Promedio[1] = -2;
    else
      Promedio[1] = 0;
    if(Promedio[2] >= distancia_min && Promedio[2] <= distancia_max)
      Promedio[2] = -3;
    else
      Promedio[2] = 0;
    if(Promedio[4] >= distancia_min && Promedio[4] <= distancia_max)
      Promedio[4] = 3;
    else
      Promedio[4] = 0;
    if(Promedio[5] >= distancia_min && Promedio[5] <= distancia_max)
      Promedio[5] = 2;
    else
      Promedio[5] = 0;
    if(Promedio[6] >= distancia_min && Promedio[6] <= distancia_max)
      Promedio[6] = 1;
    else
      Promedio[6] = 0;
